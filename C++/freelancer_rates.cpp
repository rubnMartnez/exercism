...
double apply_discount(double before_discount, double discount) {
    ...
    double actualDiscount = before_discount * discount / 100;
    return before_discount - actualDiscount;
}
...
int monthly_rate(double hourly_rate, double discount) {
    ...
    double monthlyPrice = daily_rate(hourly_rate) * 22;
    return (int)ceil(apply_discount(monthlyPrice, discount));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double dailyPrice = apply_discount(daily_rate(hourly_rate), discount);
    return (int)floor(budget / dailyPrice);
}
